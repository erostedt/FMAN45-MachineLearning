...
while 1
    ...
    while 1
        ...
        for state_idx = 1 : nbr_states 
            ...
            s_prime = next_state_idxs(state_idx, action);
    ...
    policy_stable = true; 
    for state_idx = 1 : nbr_states
        old_policy = policy(state_idx);
        action_values = zeros(3,1);
        
        for action=1:nbr_actions
            s_prime = next_state_idxs(state_idx, action);
            
            if s_prime == -1
                action_values(action) = rewards.apple; 

            elseif s_prime == 0
                 action_values(action) = rewards.death;

            else
                 action_values(action) = rewards.default + gamm*values(s_prime);
            end
        end 
        
        [~, idx] = max(action_values);
        policy(state_idx) =  idx;

        if policy_stable
            policy_stable = (old_policy == policy(state_idx));
        end
    end
